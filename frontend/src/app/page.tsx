...
function InputPanel({
  ...
  return (
    <div className={`bg-[#0B101A] border rounded-2xl p-4 sm:p-6 shadow-xl space-y-4 ${accentBorder} transition-all`}>
...
export default function AI_CV_Matcher_Pro() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#06080D] text-slate-300 font-sans overflow-y-auto md:overflow-hidden">
      {/* SIDEBAR */}
      <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-slate-800/60 bg-[#0A0D14] p-4 md:p-6 flex flex-row md:flex-col justify-between items-center md:items-stretch shrink-0 z-30">
        <div className="flex flex-row md:flex-col items-center md:items-stretch gap-4 md:gap-0 justify-between w-full md:w-auto">
          <div className="flex items-center gap-3 md:mb-10">
            <div className="w-8 h-8 rounded-lg bg-violet-600 flex items-center justify-center shrink-0">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-white font-bold text-sm md:text-lg leading-tight">MatchAI Pro</h2>
              <p className="text-[9px] md:text-[10px] text-slate-500 tracking-widest uppercase">BATCH VERSION</p>
            </div>
          </div>
          <nav className="flex flex-row md:flex-col gap-2 md:space-y-1">
            <button
              onClick={() => { setActiveTab("single"); setShowResult(false); }}
              className={`flex items-center gap-2 md:gap-3 px-3 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-all ${
                activeTab === "single"
                  ? "bg-violet-500/10 text-violet-400 border border-violet-500/20"
                  : "text-slate-500 hover:text-slate-200"
              }`}
            >
              <CopyPlus className="w-3.5 h-3.5 md:w-4" />
              <span>1-vs-1 Match</span>
            </button>
            <button
              onClick={() => { setActiveTab("batch"); setShowResult(false); }}
              className={`flex items-center gap-2 md:gap-3 px-3 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-all ${
                activeTab === "batch"
                  ? "bg-violet-500/10 text-violet-400 border border-violet-500/20"
                  : "text-slate-500 hover:text-slate-200"
            ...
            >
              <Trophy className="w-3.5 h-3.5 md:w-4" />
              <span>Leaderboard</span>
            </button>
          </nav>
        </div>
        <div className="hidden md:block p-4 bg-slate-800/20 rounded-xl border border-slate-700/50 mt-auto">
          <p className="text-[10px] text-slate-500 italic">"Ranking 50 CVs in seconds using SBERT local embedding."</p>
        </div>
      </aside>

      {/* MAIN */}
      <div className="flex-1 flex flex-col md:overflow-hidden relative">
        <header className="hidden md:flex h-16 border-b border-slate-800/60 items-center px-8 bg-[#0A0D14]/80 backdrop-blur-md">
        ...
        <main className="flex-1 overflow-y-auto pt-6 md:pt-8 pb-12 px-4 md:px-10">
          <div className="max-w-5xl mx-auto space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <InputPanel id="jd" label="Job Description" icon={<Briefcase className="w-5 h-5" />} accentClass="text-indigo-400" accentBorder="border-slate-800/60" placeholder="Paste JD..." state={jdPanel} onChange={(updates) => setJdPanel(prev => ({ ...prev, ...updates }))} />
            </div>

            <div className="flex justify-center w-full px-4">
              <button
                onClick={() => activeTab === "single" ? handleStartScreening() : handleAnalyze()}
                disabled={analyzing || generatingQuestions}
                className="w-full sm:w-auto bg-violet-600 hover:bg-violet-500 text-white shadow-xl px-8 sm:px-12 py-3.5 rounded-full flex items-center justify-center gap-3 transition-all font-bold disabled:opacity-50 text-sm sm:text-base"
              >
            ...
            {showResult && activeTab === "single" && resultData && (
              <div className="bg-[#0B101A] border border-slate-800/80 rounded-2xl p-4 sm:p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-5">
                 <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
                    <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 text-center sm:text-left">
                       ...
                       <div>
                          <div className={`inline-block px-4 py-1 rounded-full text-[10px] font-bold border ${getStatusColor(resultData.status)}`}>{resultData.status}</div>
                          <div className="text-xs text-slate-500 mt-2">Semantic Similarity: {resultData.analysis.semantic_sim}</div>
                       </div>
                    </div>
                    <div className="w-full lg:max-w-md bg-[#121124] p-4 rounded-xl border border-violet-500/10 text-xs italic text-slate-300">
        ...
        {showQAModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#0B101A] border border-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-8">
                {screeningQuestions.map((q, i) => (
                  <div key={q.id} className="space-y-4 animate-in slide-in-from-left-4" style={{ animationDelay: `${i * 100}ms` }}>
                    ...
                    {q.type === "YES_NO" ? (
                      <div className="flex flex-wrap gap-3 ml-4 sm:ml-10">
                        {["Yes", "No"].map(val => (
                          <button
                            key={val}
              ...
              <div className="p-4 sm:p-6 border-t border-slate-800 bg-slate-900/50 flex justify-end gap-4">
        ...
        {showBatchSuggModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#0B101A] border border-slate-800 w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#06080D]/30">
                {loadingBatchSugg ? (
